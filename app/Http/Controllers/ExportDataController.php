                ...
                try {
                    switch ($table) {
                        ...
                        case 'data_beswan':
                            ...
                            // Debug: Log jumlah aplikasi yang ditemukan
                            Log::info('Export data_beswan - applications found', [
                                    ...
                                    if ($app->beswan && $app->beswan->user) {
                                        ...
                                        if ($documents->count() > 0) {
                                            ...
                                            foreach ($documents as $doc) {
                                                ...
                                                $filePath = null;
                                                $originalFileName = basename($doc->file_path); // Ambil hanya nama file dari path
                                                
                                                // Coba path yang disimpan di database
                                                if ($doc->file_path && Storage::disk('public')->exists($doc->file_path)) {
                                                    $filePath = Storage::disk('public')->path($doc->file_path);
                                                }
                                                
                                                // Jika file_path tidak ada atau tidak ditemukan, coba cari di berbagai folder
                                                if (!$filePath || !file_exists($filePath)) {
                                                    $possiblePaths = [
                                                        'dokumen-wajib/identity_proof/' . $originalFileName,
                                                        'dokumen-wajib/academic_transcript/' . $originalFileName,
                                                        'dokumen-wajib/recommendation_letter/' . $originalFileName,
                                                        'dokumen-wajib/cv_resume/' . $originalFileName,
                                                        'dokumen-wajib/essay/' . $originalFileName,
                                                        'dokumen-pendukung/org_experience/' . $originalFileName,
                                                        'dokumen-pendukung/achievement/' . $originalFileName,
                                                        'dokumen-pendukung/portfolio/' . $originalFileName,
                                                        'dokumen-pendukung/other_documents/' . $originalFileName,
                                                        'dokumen-sosmed/instagram/' . $originalFileName,
                                                        'dokumen-sosmed/linkedin/' . $originalFileName,
                                                        'dokumen-sosmed/youtube/' . $originalFileName,
                                                        'dokumen-sosmed/tiktok/' . $originalFileName,
                                                        'dokumen-sosmed/twitter/' . $originalFileName,
                                                        'dokumen-sosmed/facebook/' . $originalFileName,
                                                    ];
                                                    
                                                    foreach ($possiblePaths as $pPath) {
                                                        if (Storage::disk('public')->exists($pPath)) {
                                                            $filePath = Storage::disk('public')->path($pPath);
                                                ...
                                                // Jika masih tidak ditemukan, coba cari di folder public
                                                if (!$filePath || !file_exists($filePath)) {
                                                    $publicPaths = [
                                                        public_path('storage/' . $originalFileName),
                                                        public_path('storage/dokumen-wajib/' . $originalFileName),
                                                        public_path('storage/dokumen-pendukung/' . $originalFileName),
                                                        public_path('storage/dokumen-sosmed/' . $originalFileName),
                                                    ];
                                                    
                                                    foreach ($publicPaths as $path) {
                                                        if (file_exists($path)) {
                                                            $filePath = $path;
                                                            Log::info('Found file in public path', [
                                                                'original_path' => $doc->file_path,
                                                                'found_path' => $path
                                                            ]);
                                                            break;
